...
app.get("/filter", (req, res) => {
  ...
  if (!filteredQuotes) {
    res
      .status(404)
      .json({ error: `Quote with author ${author} is not found.` });
  } else {
...
app.put("/quotes/:id", basicAuth, (req, res) => {
  ...
  if (!existingQuote) {
    res
      .status(404)
      .json({ error: "Quote to be updated not found. No quotes updated." });
  } else {
...
app.delete("/quotes/:id", basicAuth, (req, res) => {
  ...
  if (!quoteIdx === -1) {
    res
      .status(404)
      .json({ error: "Quote to be deleted not found. No quotes deleted." });
  } else {
